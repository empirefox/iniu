...
func posDb(t string) *gorm.DB {
	return db.DB.Table(t).Order("pos desc")
}

func IpById(t string, id int64) (ip IdPos, err error) {
	err = posDb(t).Where("id = ?", id).First(&ip).Error
	return
}

func IpByPos(t string, pos int64, fns ...func(*gorm.DB) *gorm.DB) (ip IdPos, err error) {
	err = posDb(t).Scopes(fns...).Where("pos = ?", pos).First(&ip).Error
	return
}
...
func IpBeforeOrAfterAnd(t string, isBefore bool, pos int64, fns ...func(*gorm.DB) *gorm.DB) ([]IdPos, error) {
	...
	ips := []IdPos{}
	err := db.DB.Table(t).Order(o).Scopes(fns...).Where(q, pos).Limit(2).Find(&ips).Error
	return ips, err
}

func IpDb(t string, fns []func(*gorm.DB) *gorm.DB) *gorm.DB {
	return posDb(t).Scopes(fns...).Select("id, pos")
}
...
func OrderIpDb(t string, isUp bool, fns []func(*gorm.DB) *gorm.DB) *gorm.DB {
	...
	return db.DB.Table(t).Order(o).Scopes(fns...).Select("id,pos")
}
...
func Exchange(t string, id1, id2 int64) error {
	...
	ips := []IdPos{}
	err := posDb(t).Where("id in (?)", []int64{id1, id2}).Find(&ips).Error
...
func NewPosUpBetween(t string, idBase, idBottom, idTop int64, fns ...func(*gorm.DB) *gorm.DB) (int64, []IdPos, error) {
	...
	err = posDb(t).Where("id = ?", idBase).First(&baseIp).Error
	if err != nil {
		return -1, nil, err
	}
	err = posDb(t).Where("id = ?", idBottom).First(&bottomIp).Error
	if err != nil {
		return -1, nil, err
	}
	err = posDb(t).Where("id = ?", idTop).First(&topIp).Error
	if err != nil {
		return -1, nil, err
	}
